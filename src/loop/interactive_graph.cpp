...
InteractiveGraph::InteractiveGraph(std::vector<OdometryFrame::Ptr> keyframes) : keyframesodom(keyframes), GraphSLAM("lm_var_cholmod"), iterations(0), chi2_before(0.0), chi2_after(0.0), elapsed_time_msec(0.0) {
  ...
  anchor_edge = nullptr;
  floor_node = nullptr;
  scManager = new SCManager();
  hfNetManager = new HFNetManager();
}
...
void InteractiveGraph::InteractiveGraphinit(std::string model)
{
  modelpath = model;

  static Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "HFNET");
  Ort::SessionOptions session_options;
  session_options.SetIntraOpNumThreads(1);
  // std::cout << "-------------------------------------cuda: " << _VAR << std::endl;
//  OrtSessionOptionsAppendExecutionProvider_CUDA(session_options, 0);

  session_options.SetGraphOptimizationLevel(GraphOptimizationLevel::ORT_ENABLE_EXTENDED);
  const char* model_path = modelpath.data();
  // Ort::Session session(env, model_path, session_options);
  session_ = std::make_shared<Ort::Session>(env, model_path, session_options);
}
...
bool InteractiveGraph::load_map_data(const std::string& directory) {
  ...
  std::cout<<"finish load_g2o"<<std::endl;
  // re-assign edge ids
  ...
  std::cout<<"finish load_keyframes"<<std::endl;
...
bool InteractiveGraph::load_map_data() {
  // load graph file
  if(!load_graph()) {
    return false;
  }
...
bool InteractiveGraph::load_keyframes() {
  for(int i = 0;i<keyframesodom.size(); i++) {
    

    InteractiveKeyFrame::Ptr keyframe = std::make_shared<InteractiveKeyFrame>(keyframesodom[i]->timestamp, keyframesodom[i]->pose, 
                                                                    keyframesodom[i]->cloud_, i, graph.get(), keyframesodom[i]->img_);
    if(!keyframe->node) {
      std::cerr << "error : failed to load keyframe!!" << std::endl;
      // std::cerr << "      : " << keyframe_dir << std::endl;
    } else {
      keyframes[keyframe->id()] = keyframe;
      vectorkeyframes.push_back(keyframe);
    }
  }
  return true;
}

bool InteractiveGraph::update_keyframe() {
  int index = keyframesodom.size()-1;
  InteractiveKeyFrame::Ptr keyframe = std::make_shared<InteractiveKeyFrame>(keyframesodom[index]->timestamp, keyframesodom[index]->pose, 
                                                    keyframesodom[index]->cloud_, index, graph.get(), keyframesodom[index]->img_);
  if(!keyframe->node) {
    std::cerr << "error : failed to load keyframe!!" << std::endl;
    // std::cerr << "      : " << keyframe_dir << std::endl;
  } else {
    ...
    vectorkeyframes.push_back(keyframe);
    #if defined X3
    Eigen::VectorXf query = inference_x3(keyframe->img_, modelpath);
    #else
    Eigen::VectorXf query = inference_img(keyframe->img_, session_);
    #endif
    std::pair p(keyframe, query);
    db.push_back(p);

    Eigen::Quaterniond quaternion(keyframe->node->estimate().rotation());
    Eigen::Vector3d trans = keyframe->node->estimate().translation();
    std::vector<double> pose_vector{trans[0],trans[1],trans[2],quaternion.x(),quaternion.y(),quaternion.z(),quaternion.w()};
    scManager->makeAndSaveScancontextAndPose(*keyframe->cloud->makeShared(), pose_vector);
  }
  return true;
}

bool InteractiveGraph::load_graph() {
  ...
  for(int i = 0;i<keyframesodom.size(); i++) {
    ...
    int index = i;
    v->setId(index);
    v->setEstimate(keyframesodom[i]->pose);
    graph->addVertex(v);
  }
  for(int i = 0; i < keyframesodom.size() - 1; i++) {
    ...
    const auto& delta_pose = keyframesodom[i]->pose.inverse() * keyframesodom[i+1]->pose;
...
bool InteractiveGraph::update_graph() {
  g2o::SparseOptimizer* graph = dynamic_cast<g2o::SparseOptimizer*>(this->graph.get());
  //add vertex
  int index = keyframesodom.size()-1;
  g2o::VertexSE3 *v = new g2o::VertexSE3();
  v->setId(index);
  v->setEstimate(keyframesodom[index]->pose);
  graph->addVertex(v);
  //add edge
  if(index-1>=0)
  {
    ...
    e->setVertex(0, graph->vertices()[idx1]);
    e->setVertex(1, graph->vertices()[idx2]);
    const auto& delta_pose = keyframesodom[idx1]->pose.inverse() * keyframesodom[idx2]->pose;
    e->setMeasurement(delta_pose);
    Eigen::MatrixXd inf = Eigen::MatrixXd::Identity(6, 6);
    inf.block<3, 3>(0, 0) *= 10.0;
    inf.block<3, 3>(3, 3) *= 20.0;
...
void InteractiveGraph::optimize(int num_iterations) {
  ...
  isopt = false;
  optlastid = vectorkeyframes.size() - 1;
}
...
void SaveScanContextRelocDB(SCManager *scManager, std::string f)
{
  // std::cout<<"aaaaaaaaaaaaaaaa"<<std::endl;
    std::string filename = f;
    std::ofstream out(filename, std::ios_base::binary);

    std::cout << "Saving Scan Context DBfile: " << filename << std::flush;
...
void SaveHFNetRelocDB(HFNetManager *hfNetManager, std::string f)
{
  // std::cout<<"aaaaaaaaaaaaaaaa"<<std::endl;
    std::string filename = f;
    std::ofstream out(filename, std::ios_base::binary);

    std::cout << "Saving HFNet DBfile: " << filename << std::flush;
    boost::archive::binary_oarchive oa(out, boost::archive::no_header);
...
bool InteractiveGraph::save_pointcloud(const std::string& filename) {
  std::ofstream f1;
  f1.open(filename+"posemap.txt");
  f1 << std::fixed;
  // pcl::PointCloud<pcl::PointXYZI>::Ptr accumulated(new pcl::PointCloud<pcl::PointXYZI>());
  int index = 0;
  for(const auto& keyframe : vectorkeyframes) {

    // pcl::PointCloud<pcl::PointXYZI>::Ptr transformed(new pcl::PointCloud<pcl::PointXYZI>());
    // pcl::transformPointCloud(*keyframe->cloud, *transformed, keyframe->node->estimate().cast<float>());
    // pcl::io::savePCDFileBinary(filename+std::to_string(keyframe->id())+".pcd", *transformed);
    Eigen::Quaterniond quaternion(keyframe->node->estimate().rotation());
    Eigen::Vector3d trans = keyframe->node->estimate().translation();
    std::vector<double> pose_vector{trans[0],trans[1],trans[2],quaternion.x(),quaternion.y(),quaternion.z(),quaternion.w()};
    // std::ofstream f;
    // f.open(filename+std::to_string(keyframe.second->timestamp)+".txt");
    // f<<trans[0]<<" "<<trans[1]<<" "<<trans[2]<<" "<<quaternion.x()<<" "<<quaternion.y()<<" "<<quaternion.z()<<" "<<quaternion.w()<<std::endl;
    // f.close();
    // std::copy(transformed->begin(), transformed->end(), std::back_inserter(accumulated->points));
    f1 << std::setprecision(6) << keyframe->timestamp << " " << std::setprecision(9) << trans[0] << " " << trans[1] << " " << trans[2] << " " << quaternion.x() << " " << quaternion.y() << " " << quaternion.z() << " " << quaternion.w() << std::endl;
    scManager->makeAndSaveScancontextAndPose(*keyframe->cloud->makeShared(), pose_vector);
    hfNetManager->saveHFNetDataBase(*keyframe->cloud->makeShared(), keyframe->img_, db[index].second, pose_vector);
    index++;
  }
  f1.close();
  SaveScanContextRelocDB(scManager,filename+"ScanContext.bin");
  SaveHFNetRelocDB(hfNetManager,filename+"HFNet.bin");

  // accumulated->is_dense = false;
  // accumulated->width = accumulated->size();
  // accumulated->height = 1;
  // pcl::io::savePCDFileBinary(filename+"cloudmodel.pcd", *accumulated);
  return true;
}

pcl::PointCloud<pcl::PointXYZI>::Ptr InteractiveGraph::get_pointcloud()
{
  pcl::PointCloud<pcl::PointXYZI>::Ptr accumulated(new pcl::PointCloud<pcl::PointXYZI>());
  for(const auto& keyframe : keyframes) {
