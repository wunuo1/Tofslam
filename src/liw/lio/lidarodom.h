...
namespace zjloc
{
     struct liwOptions
     {
          ...
          int wheelmode;
          int use_ground_constraint;

          std::string mapfile; 
          std::string scancontext_dbfile;
          std::string hfnet_dbfile;
          int reloc_mode;
     ...
     public:
          lidarodom(/* args */);
          // lidarodom(Loop *loop_tmp);
          ~lidarodom();

          bool init(const std::string &config_yaml);

          // void pushData(std::vector<point3D>, std::pair<double, double> data);
          void pushData(std::vector<point3D>, std::pair<double, double> data, rclcpp::Time time, pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_tmp);
          void pushData(std::vector<point3D>, std::pair<double, double> data, rclcpp::Time time);
          void pushData(cv::Mat msg, double timestamp, rclcpp::Time time);
          void pushData(IMUPtr imu);
          void pushData(OdomposPtr odom);

          void run();

          int getIndex() { return index_frame; }

          void setFunc(std::function<bool(std::string &topic_name, CloudPtr &cloud, double time, int mode)> &fun) { pub_cloud_to_ros = fun; }
          // void setFunc(std::function<bool(std::string &topic_name, CloudPtr &cloud, double time, int distort)> &fun) { pub_cloud_to_ros2 = fun; }
          void setFunc(std::function<bool(std::string &topic_name, SE3 &pose, SE3 & poseinrobot, double time)> &fun) { pub_pose_to_ros = fun; }
          // void setFunc(std::function<bool(std::string &topic_name, SE3 &pose, SE3 & poseinrobot, ros::Time time)> &fun) { pub_pose_to_ros = fun; } 
          void setFunc(std::function<bool(std::string &topic_name, double time1, double time2)> &fun) { pub_data_to_ros = fun; }

          void setMode(bool ol);
     ...
     private:
          ...
          void relocInitialization(std::vector<point3D>& cloud);

          void relocInitialization(std::vector<point3D>& cloud, cv::Mat& img_mat);

          cloudFrame *buildFrame(std::vector<point3D> &const_surf, state *cur_state,
                                 double timestamp_begin, double timestamp_end);

          cloudFrame *buildFrame(std::vector<point3D> &const_surf, state *cur_state,
                                 double timestamp_begin, double timestamp_end, Eigen::VectorXf lidar_coeff);
          ...
          void loadmap(pcl::PointCloud<pcl::PointXYZI>::Ptr pcl_points);
     ...
     // const cloudFrame *p_frame, std::vector<Eigen::Vector3d> &normals,  double kMaxPointToPlane);
          ...
          void loadScanContextRelocDB();

          void loadHFNetRelocDB();
     ...
     private:
          ...
          std::function<bool(std::string &topic_name, CloudPtr &cloud, double time, int mode)> pub_cloud_to_ros;
          // std::function<bool(std::string &topic_name, CloudPtr &cloud, double time, bool distort)> pub_cloud_to_ros2;
          // std::function<bool(std::string &topic_name, CloudPtr &cloud, double time, bool distort)> pub_cloud_to_ros3;
          std::function<bool(std::string &topic_name, SE3 &pose, SE3 & poseinrobot, double time)> pub_pose_to_ros;
          // std::function<bool(std::string &topic_name, SE3 &pose, SE3 & poseinrobot, ros::Time time)> pub_pose_to_ros;
          ...
          pcl::PointCloud<pcl::PointXYZI>::Ptr points_distort;
          // Vec3d Robot_T_wrt_IMU;
          // Mat3d Robot_R_wrt_IMU;
          SE3 last_pose_of_odom;
          SE3 last_pose_of_liwo;
          SE3 right_pose_of_odom;
          SE3 right_pose_of_liwo;
          ...
          int optnum=0;
          bool online=false;
          std::shared_ptr<InteractiveGraph> graph_;
          bool updategraph=false;
          Eigen::Isometry3d optdeltapose;
